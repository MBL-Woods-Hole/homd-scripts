...
#         txt += blast_cmd % (args.full_blast_db, filepath, filepath, blast_outfmt)
#         f.write(txt)
#         n += 1
#        
#     
...
def run_blast(args):
    # find all the *.fa file in args.indir
    faFileNames = [f for f in os.listdir(args.indir) if f.endswith('.fa')]
    print(faFileNames)
    f = open(blast_script_path, "w")
    txt = "#!/bin/bash\n\n"
    f.write(txt)
...
def run_parse(args):
    ...
    for filename in os.listdir(args.outdir):
        if filename.endswith(".out"):  # blast files
    ...
    oligo_arry.sort()
    for oligo in oligo_arry:  # each sequence
...
#             default=False,  help=" ")
    parser.add_argument("-i", "--indir",   required=False,  action="store",   dest = "indir", 
            default='./',  help=" ")
    ...
    parser.add_argument("-db", "--blast_db", required = False, action = 'store', dest = "full_blast_db", 
            default = 'HOMD_16S_rRNA_RefSeq_V15.22.p9.fasta', help = "")
    ...
    if args.verbose:
        print(blast_cmd % (args.full_blast_db, 'queryseq.fa', 'queryseq.fa', blast_outfmt))
