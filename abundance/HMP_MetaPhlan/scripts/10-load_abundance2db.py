...
def run_abundance_csv(args): 
    ...
    if args.source == 'eren_v1v3':
        ...
        tmp = 'BMU-mean,BMU-sd,AKE-mean,AKE-sd,HPA-mean,HPA-sd,THR-mean,THR-sd,PTO-mean,PTO-sd,TDO-mean,TDO-sd,SAL-mean,SAL-sd,SUPP-mean,SUPP-sd,SUBP-mean,SUBP-sd,STO-mean,STO-sd'
        ...
        # new set of acronyms
        check = 'Max'
        l = ['AKE-mean','AKE-10p','AKE-90p','AKE-sd','AKE-prev',
             'ANA-mean','ANA-10p','ANA-90p','ANA-sd','ANA-prev',
             'BMU-mean','BMU-10p','BMU-90p','BMU-sd','BMU-prev',
             'HPA-mean','HPA-10p','HPA-90p','HPA-sd','HPA-prev',
             'LAF-mean','LAF-10p','LAF-90p','LAF-sd','LAF-prev',
             'LRC-mean','LRC-10p','LRC-90p','LRC-sd','LRC-prev',
             'MVA-mean','MVA-10p','MVA-90p','MVA-sd','MVA-prev',
             'PFO-mean','PFO-10p','PFO-90p','PFO-sd','PFO-prev',
             'PTO-mean','PTO-10p','PTO-90p','PTO-sd','PTO-prev',
             'RAF-mean','RAF-10p','RAF-90p','RAF-sd','RAF-prev',
             'RRC-mean','RRC-10p','RRC-90p','RRC-sd','RRC-prev',
             'SAL-mean','SAL-10p','SAL-90p','SAL-sd','SAL-prev',
             'STO-mean','STO-10p','STO-90p','STO-sd','STO-prev',
             'SUBP-mean','SUBP-10p','SUBP-90p','SUBP-sd','SUBP-prev',
             'SUPP-mean','SUPP-10p','SUPP-90p','SUPP-sd','SUPP-prev',
             'THR-mean','THR-10p','THR-90p','THR-sd','THR-prev',
             'TDO-mean','TDO-10p','TDO-90p','TDO-sd','TDO-prev',
             'VIN-mean','VIN-10p','VIN-90p','VIN-sd','VIN-prev']
        tmp = ','.join(l)
        reference = args.source
        rank = 'species'
    elif args.source == 'HMP_MetaPhlan':
        # calulate max mead
        # new set of acronyms
        check = 'Max'
        l = ['AKE-mean','AKE-10p','AKE-90p','AKE-sd','AKE-prev',
             ...
             'LRC-mean','LRC-10p','LRC-90p','LRC-sd','LRC-prev',
             'MVA-mean','MVA-10p','MVA-90p','MVA-sd','MVA-prev',
             'PERIO-mean','PERIO-10p','PERIO-90p','PERIO-sd','PERIO-prev',
             'PFO-mean','PFO-10p','PFO-90p','PFO-sd','PFO-prev',
             'PTO-mean','PTO-10p','PTO-90p','PTO-sd','PTO-prev',
             'RAF-mean','RAF-10p','RAF-90p','RAF-sd','RAF-prev',
             'RRC-mean','RRC-10p','RRC-90p','RRC-sd','RRC-prev',
    ...
    print('active',active)
    # if reference is already in db then we UPDATE Not INSERT
    # qref = "SELECT DISTINCT reference from `abundance_NEW`"
#     res = myconn_new.execute_fetch_select(qref)
#     refs = [x[0] for x in list(res)]
#     print(refs)
    ...
    with open(args.infile) as csv_file: 
        ...
        for row in csv_reader:
            ...
            q = "INSERT IGNORE INTO `"+args.sql_table+"` (reference,otid,"+','.join(rankid_list)+",notes,`level`,`max`,`"+'`,`'.join(active_clean)+"`) VALUES "
            if not row[check]:
               continue
            ...
            if row['Taxonomy'] in ['NonHOMD','LowAbund','UNMATCHED']:
                continue
            print(row['Taxonomy'])
...
if __name__ == "__main__":
    ...
    USAGE:
        ...
        USE 10-load_DewhirstErenAbundance2db.py for dewhirst and Eren
        ...
        -src/--source ['NIH_v1v3','NIH_v3v5']
        -t--sqltable  DEFAULT abundance_NEW
    ...
    parser.add_argument("-v", "--verbose",   required=False,  action="store_true",    dest = "verbose", default=False,
                                                    help="verbose print()") 
    parser.add_argument("-t", "--sqltable",
                        required = False, action = 'store', dest = "sql_table", default = 'abundance_NEW',
                        help = "")
                        
    args = parser.parse_args()
    args.source = 'HMP_MetaPhlan'
    ...
    'BMU',  #Buccal_mucosa
    'HPA',  #Hard_palate
    #'LAF',  #L_Antecubital_fossa  Not with HMP
    'PERIO',  # only with HMP
