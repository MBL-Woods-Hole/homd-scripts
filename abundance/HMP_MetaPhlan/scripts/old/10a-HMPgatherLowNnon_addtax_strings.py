...
def run():  
    ...
    for taxon in master:
        ...
        if max_abund <= float(args.low_abund) or taxon == 'z__Low_abundance':
            low_abund_rows[taxon] = master[taxon]
...
#             file_pointers['rows_other_low_abund'].write('\n')
            ...
            if sql_result:
                if len(sql_result) > 1:   #  Has subsp
                    ...
                    tax_strings[taxon] = sql_result[0][1:8]
                    level = 'species'
                else:
                    hmt = sql_result[0][0]
                    notes = ''
                    #tax_string = ';'.join(sql_result[1:])
                    tax_strings[taxon] = sql_result[0][1:8] # includes hmt[0] and subsp[-1]
                    level = 'species'

                tstr = ';'.join(tax_strings[taxon])
                tstr = tstr.rstrip(';')
                outfilep.write(tstr+'\t'+taxon+'\t'+str(hmt)+'\t'+level+'\t'+notes)
                for i,sample in enumerate(new_ds_names):
                    outfilep.write('\t'+master[taxon][i])
                outfilep.write('\n')
            else:
                ...
                if res and res[0]:
                    tax_string = ';'.join(res)
                    tax_strings[taxon] = tax_string
                    # write to rows_wGenus_in_homd
                    outfilep.write(tax_strings[taxon]+'\t'+taxon+'\t'+str(hmt)+'\tgenus'+'\t'+notes)
                    
                    for i,sample in enumerate(new_ds_names):
                        outfilep.write('\t'+master[taxon][i])
                    outfilep.write('\n')
                else:
                    non_homd_rows[taxon] = master[taxon]
    #Other
    new_row_non = [0] * len(new_ds_names)
    sum_col0 = 0
    for taxon in non_homd_rows:
        # summing
        if taxon == 'GGB1574_SGB2161':
            print('GGB1574_SGB2161',master[taxon][0],master[taxon][1])
        for i,sample in enumerate(new_ds_names):
            if i == 0:
                sum_col0 += float(master[taxon][0])
            new_row_non[i] += float(master[taxon][i])
    ...
    for taxon in low_abund_rows:
        
        #add_row_by_cell()
        for i,sample in enumerate(new_ds_names):
            new_row_low[i] += float(master[taxon][i])
