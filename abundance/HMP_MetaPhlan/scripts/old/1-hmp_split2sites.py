...
def run():  
    ...
    #open 18 files
    outfiles = {}
    for site in site_conversion.values():  #'AKE,BRU...'
        fn = site+'/'+site+'_'+args.outfile
        fnp = open(fn,'w')
        outfiles[site] = fnp
    ...
    for site in outfiles:  #'AKE,BRU...'
        outfiles[site].write('Taxon\tHMT')
        
    # row1 => write the sample name into the correct file
    for sample in new_ds_names:
        for site in outfiles:
            if site in sample:
                outfiles[site].write('\t'+sample)
    ...
    for taxon in master:
        ...
        #print('hmt',hmt)
        for site in outfiles:
            outfiles[site].write(taxon+'\t'+str(hmt))
        for i,sample in enumerate(new_ds_names):
            for site in outfiles:
                if site in sample:
                    outfiles[site].write('\t'+master[taxon][i])
