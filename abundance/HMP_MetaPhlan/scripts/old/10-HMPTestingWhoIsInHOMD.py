...
def run():  
    ...
    for item in file_pointers:
        file_pointers[item].write('Taxon\tHMT')
        if not args.taxa_only:
            for sample in new_ds_names:
                file_pointers[item].write('\t'+sample)
        file_pointers[item].write('\n')
    
    # other rows
    for taxon in master:
        #print('max',max([float(x) for x in master[taxon]]))
        max_abund = max([float(x) for x in master[taxon]])
        pts = taxon.split('_')
        #print(name)
        hmt = find_hmt(taxon)
        #print('hmt',hmt)
        if max_abund <= float(args.low_abund) or taxon == 'z__Low_abundance':
            ...
            if hmt:
                file_pointers['rows_wHOTs'].write(taxon+'\t'+str(hmt))
                if not args.taxa_only:
                    for i,sample in enumerate(new_ds_names):
                        file_pointers['rows_wHOTs'].write('\t'+master[taxon][i])
                file_pointers['rows_wHOTs'].write('\n')
            else:
            
                res = find_genus_in_homd(pts[0])
                #print('genus',pts,'res',res)
                if res and res[0]:
                    # write to rows_wGenus_in_homd
                    file_pointers['rows_wGenus_in_homd'].write(taxon+'\t'+str(hmt))
                    if not args.taxa_only:
                        for i,sample in enumerate(new_ds_names):
                            file_pointers['rows_wGenus_in_homd'].write('\t'+master[taxon][i])
                    file_pointers['rows_wGenus_in_homd'].write('\n')
                else:
                    file_pointers['rows_wGenus_not_in_homd'].write(taxon+'\t'+str(hmt))
                    if not args.taxa_only:
                        for i,sample in enumerate(new_ds_names):
                            file_pointers['rows_wGenus_not_in_homd'].write('\t'+master[taxon][i])
                    file_pointers['rows_wGenus_not_in_homd'].write('\n')
            
            # file_pointers['rows_other'].write(taxon+'\t'+str(hmt))
#             for i,sample in enumerate(new_ds_names):
#                 file_pointers['rows_other'].write('\t'+master[taxon][i])
#             file_pointers['rows_other'].write('\n')
...
V_VI_SRS…    
    ...
    print('args.taxa_only',args.taxa_only)
...
"""
    ...
    myconn = MyConnection(host=args.dbhost, db=args.DATABASE,  read_default_file = "~/.my.cnf_node")
