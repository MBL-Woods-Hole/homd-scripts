...
def run(args):
    ...
    with open(args.infile) as csv_file: 
        ...
        for row in csv_reader:
            ...
            otid = get_otid_from_hmt(row['HOT-ID'])
            hmt = row.pop('HOT-ID', None)
            num = row.pop('num', None)
            pct = row.pop('pct', None)
            if otid:
                taxonomy = get_taxonomy_from_db(otid)
                #taxonomy = row.pop('Taxonomy', None)
                if otid == '578':
                    # Bacteria;Bacillota;Bacilli;Lactobacillales;Streptococcaceae;Streptococcus;cristatus;clade_578
                    print('found 578 subsp',taxonomy)
                #print('row',hmt,row)
                taxlookup[taxonomy] = row
                otidlookup[taxonomy] = {'HMT':otid}
            if hmt == 'Unmatched':
                unmatched = row
