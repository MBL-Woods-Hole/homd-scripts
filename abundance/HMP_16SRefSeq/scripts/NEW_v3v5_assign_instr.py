...
#             outfp.write('\n')
    ...
    for hmts in multiples_lookup:
        ...
        if len(hmts_lst) == 2:
            ...
            h1 = hmts_lst[0]
            h2 = hmts_lst[1]
            if h1 in args.suppressed_singles:
                print('h1suppressing single',h1,hmts_lst)
                continue
            if h2 in args.suppressed_singles:
                print('h2suppressing single',h2,hmts_lst)
                continue
            notes[h1] = []
            notes[h2] = []
            h1in = False
        ...
        if len(hmts_lst) > 2:
            ...
            if len(new_hmts_list) == 0:  # means that none are in singles list
                ...
                for hmtcode in hmts_lst:
                    if hmtcode in args.suppressed_singles:
                        print('suppressing single',hmtcode)
                        continue
    ...
    if os.path.exists(pctfile):
        ...
        for line in fp:
            ...
            if line_pts[0].startswith('HMT-'):
                ...
                if 'HMT-'+msid in toSuppress:
                    #print('supp from pct',msid)
                    continue
                #print('XXX',msid)
