...
def get_taxonomy_ids_from_db(otid):
    sql = "select taxonomy_id,domain,domain_id,phylum,phylum_id,klass,klass_id,`order`,order_id,family,family_id,genus,genus_id,species,species_id,subspecies,subspecies_id from otid_prime"
    #sql = "select domain,phylum,klass,`order`,family,genus,species from otid_prime"
    sql += " join taxonomy using(taxonomy_id)"
    sql += " join domain using(domain_id)"
    sql += " join phylum using(phylum_id)"
    ...
    q1 = sql % (otid)
    #print(q1)
    result = myconn.execute_fetch_select_dict(q1)
    print('result',result[0])
    result_dict = result[0]
    tax_id = str(result_dict['taxonomy_id'])  # returns AND removes taxonomy_id
    print('result_dict',result_dict)
    if result_dict['subspecies_id'] != 1:
        # join sp and subsp
        result_dict['species'] = result_dict['species'] +'$$$'+result_dict['subspecies']
        
    taxstr = result_dict['domain']+';'+result_dict['phylum']+';'+result_dict['klass']+';'+result_dict['order']+';'+result_dict['family']+';'+result_dict['genus']+';'+result_dict['species']
    taxidstr = str(result_dict['domain_id'])+';'+str(result_dict['phylum_id'])+';'+str(result_dict['klass_id'])+';'+str(result_dict['order_id'])+';'+str(result_dict['family_id'])+';'+str(result_dict['genus_id'])+';'+str(result_dict['species_id'])+';'+str(result_dict['subspecies_id'])
    print('taxstr',taxstr)
    print('taxidstr',taxidstr)
    sys.exit()
    return (tax_id,taxstr,taxidstr)
def run(args):
    ...
    with open(args.infile) as csv_file: 
        ...
        for row in csv_reader:
            ...
            if hmt == 'Unmatched':
                ...
                (tax_id, taxonomy,taxidstr) = get_taxonomy_ids_from_db(otid)
