...
#                     subsp_rows.append(row)
            ...
            #taxa = row.pop('Taxa', None)
            #lookup[row['Taxonomy']]['Rank']= 'XX' # gets set elsewhere
            
            if hmt in lookup:
                sys.exit('error: hmt already in lookup')
            lookup[hmt] = {}
            note_lookup[hmt] = notes
            #lookup[row['Taxonomy']]['Note']= row['Notes']
            # if hmt in hmt_w_subspecies:
#                 subsp_rows.append(row) 
            rowmax = 0
            for site in sites:
                #print(site,row)
                mean = calc(row, site, 'mean')
                
                if mean > rowmax:
                    rowmax = mean
                lookup[hmt][site+'-mean']  = mean
                lookup[hmt][site+'-sd']    = calc(row, site, 'sd')
                lookup[hmt][site+'-10p']   = calc(row, site, '10p')
                lookup[hmt][site+'-90p']   = calc(row, site, '90p')
                
                lookup[hmt][site+'-prev'] = calc(row, site, 'prev')
            lookup[hmt]['Max'] = rowmax
...
#         lookup[species]['Max'] = rowmax
    ...
    for hmt in lookup:
        ...
        txt =  hmt
        txt += '\t'+note_lookup[hmt]
        #txt += '\t'+tax  #lookup[tax]['Rank']
        
        txt += '\t'+str(round(lookup[hmt]['Max'],5))
        for site in sites:
            txt += '\t'+str(round(lookup[hmt][site+'-mean'],5))
            txt += '\t'+str(round(lookup[hmt][site+'-10p'],5))
            txt += '\t'+str(round(lookup[hmt][site+'-90p'],5))
            txt += '\t'+str(round(lookup[hmt][site+'-sd'],5))
            txt += '\t'+str(round(lookup[hmt][site+'-prev'],5))
...
if __name__ == "__main__":
    ...
    parser.add_argument("-outfile", "--out_file", required = False, action = 'store', dest = "outfile", 
            default = 'MeanStdevPrev_forJMW', help = "")
