...
def run():  
    ...
    for line in mtx:
        ...
        if line_pts[0] == '#OTU ID':
            header = line_pts[1:]
            if print_taxa:
                foutClean.write('HOT-ID\tTaxa\t'+'\t'.join(header)+'\n')
            else:
                foutClean.write('HOT-ID\t'+'\t'.join(header)+'\n')
            for ds in header:
        ...
        if 'Bacteria' in line_pts[0] or line_pts[0].startswith('Archaea'):
            ...
            sumrowcounts = sum([float(x) for x in counts_ary])
            if sumrowcounts > 0:
    ...
    #                     split_text = ','.join(split_text_lst)
                        ...
                        for tax in multi_file_collector[msid]['taxa']:
                            ...
                            multihmt.append(args.hot_conversion[tax]['hmt'].zfill(3))
                            multitax.append(tax)
                        ...
                        #print('hmts',hmts)
                        if print_taxa:
                            foutClean.write('HMT-'+hmts+'\t'+taxa+'\t'+'\t'.join(counts_ary)+'\n')
                        else:
                            foutClean.write('HMT-'+hmts+'\t'+'\t'.join(counts_ary)+'\n')
                    else:
                    ...
                    if tax not in args.hot_conversion:
                        ...
                        hmt = args.hot_conversion[tax]['hmt'].zfill(3)
                    #for hmt in 
                    #print('hmt',hmt)
                    if print_taxa:
                        foutClean.write('HMT-'+hmt+'\t'+tax+'\t'+'\t'.join(counts_ary)+'\n')
                    else:
                        foutClean.write('HMT-'+hmt+'\t'+'\t'.join(counts_ary)+'\n')
    if print_taxa:
        foutClean.write('Unmatched\tUnmatched\t')
    else:
        foutClean.write('Unmatched\t')
    nov_counts = []
    for ds in header:
...
THIS SCRIPT  ==>        2-separate_multispecies_taxa.py
    ...
    args.region = ifpts[1]
    args.outfile = '_'.join([args.site,args.region,'counts','for_JMW',today+'.csv'])
