...
#                 subsp_rows.append(row) 
            ...
            for site in sites:
                ...
                if mean > rowmax:
                    rowmax = mean
                lookup[hmts][site+'-mean']  = mean
                lookup[hmts][site+'-sd']    = calc(row, site, 'sd')
                lookup[hmts][site+'-10p']   = calc(row, site, '10p')
                lookup[hmts][site+'-90p']   = calc(row, site, '90p')
                
                lookup[hmts][site+'-prev'] = calc(row, site, 'prev')
            lookup[hmts]['Max'] = rowmax
...
#         lookup[species]['Max'] = rowmax
    ...
    for hmts in lookup:
        ...
        txt += '\t'+str(round(lookup[hmts]['Max'],5))
        for site in sites:
            txt += '\t'+str(round(lookup[hmts][site+'-mean'],5))
            txt += '\t'+str(round(lookup[hmts][site+'-10p'],5))
            txt += '\t'+str(round(lookup[hmts][site+'-90p'],5))
            txt += '\t'+str(round(lookup[hmts][site+'-sd'],5))
            txt += '\t'+str(round(lookup[hmts][site+'-prev'],5))
