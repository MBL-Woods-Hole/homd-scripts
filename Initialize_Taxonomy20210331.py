...
h.NCBI_taxon_id as NCBI_taxon_id
FROM    {tbl0} a
LEFT JOIN {tbl1} b
...
def run_taxa(args):
    ...
    for obj in result:
        ...
        if obj['otid'] not in master_lookup:
            #taxonObj = Taxon(obj['otid']) 
            master_lookup[obj['otid']] = {}
            master_lookup[obj['otid']]['genomes'] = []
            master_lookup[obj['otid']]['type_strain'] = []
            master_lookup[obj['otid']]['ref_strain'] = []
            master_lookup[obj['otid']]['synonyms'] = []
            master_lookup[obj['otid']]['site'] = []
        for n in obj:
            print('n',n)
            toadd = str(obj[n]).strip()
            print('synonyms',obj['synonyms'])
            print('type_strain',obj['type_strain'])
            
            if n =='type_strain':  #list
                #lookup[obj['otid']][n] = str(obj[n]).strip().replace('"','').replace("'","").replace(',','').split(split_code)
                if 'type_strain' not in master_lookup[obj['otid']]:
                    master_lookup[obj['otid']]['type_strain'] = []
                if toadd not in master_lookup[obj['otid']]['type_strain']:
                    master_lookup[obj['otid']]['type_strain'].append(toadd)
           
            elif n=='ref_strain':  #list  
                if 'ref_strain' not in master_lookup[obj['otid']]:
                    master_lookup[obj['otid']]['ref_strain'] = []
                if toadd not in master_lookup[obj['otid']]['ref_strain']:
                    master_lookup[obj['otid']]['ref_strain'].append(toadd)  
            
            elif n=='synonyms':  #list
                #lookup[obj['otid']][n] = str(obj[n]).strip().replace('"','').replace("'","").replace(',','')
                if 'synonyms' not in master_lookup[obj['otid']]:
                    master_lookup[obj['otid']]['synonyms'] = []
                if toadd not in master_lookup[obj['otid']]['synonyms']:
                    master_lookup[obj['otid']]['synonyms'].append(toadd)  
                
            elif n=='site':  #list
                #lookup[obj['otid']][n] = str(obj[n]).strip().replace('"','').replace("'","").replace(',','').split(split_code)
                if 'site' not in master_lookup[obj['otid']]:
                    master_lookup[obj['otid']]['site'] = []
                if toadd not in master_lookup[obj['otid']]['site']:
                    master_lookup[obj['otid']]['site'].append(toadd) 
               
            else:
                master_lookup[obj['otid']][n] = toadd.replace('"','').replace("'","").replace(',','')
            
    # TODO  typestrain and synonyms need to <listed> in lookup
    ## printing is done later
...
def run_get_genome_count(args):  ## add this data to master_lookup
    ...
    for obj in result:
        #print(obj)
        if obj['otid'] not in master_lookup:
            print('Did not find otid in object! - this needs attention! ('+str(obj['otid'])+')  -Continuing')
            continue
            #master_lookup[obj['otid']] = {}
            
        if 'genomes' not in master_lookup[obj['otid']]:
            master_lookup[obj['otid']]['genomes'] = []
