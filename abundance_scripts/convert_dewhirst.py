...
if __name__ == "__main__":
    ...
    USAGE:
       ...
       ./convert dewhirst.py -i dewhirst_edit2021-12-24.csv
       
       from header Taxonomy        comment HMT     Rank    type    source  Max     max any oral site       Abundance category      BM-mean BM-sd 
       
       to header: HMT     NOTES   BM-MEAN BM-SD   BM-PREV HP-MEAN HP-SD 
    ...
    parser.add_argument("-outfile", "--out_file", required = False, action = 'store', dest = "outfile", 
            default = 'dewhirstFINAL', help = "")
    
    args = parser.parse_args()
    
    if args.source not in ['dewhirst_35x9']:
        print(usage)
        sys.exit()
