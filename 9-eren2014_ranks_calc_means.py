    ...
    with open(args.infile) as csv_file: 
        ...
        for row in csv_reader:
            ...
            for site in site_order:
                lookup[row['Taxonomy']][site+'-MEAN'] = calc(row, site, 'mean')
                lookup[row['Taxonomy']][site+'-SD']   = calc(row, site, 'sd')
                lookup[row['Taxonomy']][site+'-PREV'] = calc(row, site, 'prev')
            for key in row.keys():
                items = key.split('-')
...
if __name__ == "__main__":
    ...
    USAGE:
       ...
       ../eren2014_calc_means.py -i HOMD_NEWcoalesce01.csv   
    ...
    parser.add_argument("-outfile", "--out_file", required = False, action = 'store', dest = "outfile", 
            default = 'MeanStdevPrev_byRank', help = "")
